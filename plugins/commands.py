        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAKdmGfuXkIvIncW2UPVAAHB0Ma_Ze_ovQACow8AAi-wCFRpLdczFHMQWx4E") 
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/pkbotssupport1</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/pkbotssupport1').json()
...
async def refer(bot, message):
    ...
    m=await message.reply_sticker("CAACAgUAAxkBAAEBKvRnRzWGVdZDR6SdYAvdEIoX3MqFtgACowAD7xR5MGsgg9doKpQmHgQ")      
    await m.delete()
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
